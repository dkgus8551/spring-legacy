...
public class Utils {
	...
	public static String saveFile(MultipartFile upfile, ServletContext application, String boardCode) {
		...
		// getRealPath(경로)
		//  - 실제 서버의 파일 시스템 경로를 절대 경로로 반환하는 메소드
		// ex) C:/springWorkspace/springProject/...
	...
	/*
	 * XSS(크로스 사이트 스크립트) 공격을 방지하기 위한 메소드
	 *  - 스크립트 삽입 공격
	 *  - 사용자가 <script> 태그를 게시글에 작성하여 클라이언트가 게시글 클릭 시
	 *    script에 지정한 코드가 실행되도록 유도하는 방식
	 *  - 위 내용을 그대로 DB에 저장 후 브라우저에 렌더링하면 문제가 발생할 수 있으므로
	 *    태그가 아닌 기본 문자열로 인식할 수 있게끔 html 내부 entity로 변환 처리를 수행
	 * */
