...
@RequestMapping("/chat")
@Slf4j
@SessionAttributes({"chatRoomNo"})// model 영역에 추가하는 key 값이 chatRoomNo면 데이터를 HttpSession에 저장
public class ChatController {
	...
	public String openChatRoom(
		...
		/*
		 * 1. 유효성 검사(생략)
		 * 2. 테이블에 데이터 추가를 위해 room을 서비스로 전달(서비스 로직 호출)
		 * 3. 처리 결과에 따른 view 페이지 지정
		 * */
	...
	public String joinChatRoom(
		...
		/*
		 * 업무 로직
		 * 1. 채팅방 번호를 기준으로 채팅방 메시지 내용 조회
		 * 2. 참여자 수 증가
		 * 3. 채팅방 메시지를 model에 추가 후 forward
		 * */
		...
		if(list == null) {
			throw new RuntimeException("채팅방 접속 오류");
		}
